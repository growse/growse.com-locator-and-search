...
GITHUB_RUN_NUMBER ?= 0
DEBVERSION := $(VERSION:v%=%)-$(GITHUB_RUN_NUMBER)
...
package: $(addsuffix .deb, $(addprefix $(PKGNAME)_$(VERSION)-$(GITHUB_RUN_NUMBER)_, $(foreach a, $(ARCH), $(a))))

$(PKGNAME)_$(VERSION)-$(GITHUB_RUN_NUMBER)_%.deb: dist/www-growse-com_linux_%
	chmod +x $<
