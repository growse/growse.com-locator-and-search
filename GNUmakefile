...
GITHUB_RUN_ID ?= 0
DEBVERSION := $(VERSION:v%=%)-$(GITHUB_RUN_ID)
...
package: $(addsuffix .deb, $(addprefix $(PKGNAME)_$(VERSION)-$(GITHUB_RUN_ID)_, $(foreach a, $(ARCH), $(a))))

$(PKGNAME)_$(VERSION)-$(GITHUB_RUN_ID)_%.deb: dist/www-growse-com_linux_%
	chmod +x $<
